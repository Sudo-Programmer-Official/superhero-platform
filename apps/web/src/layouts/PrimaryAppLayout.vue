<template>
  ...
  <div v-else class="app-shell">
    ...
    <RouterLink class="fab-create" to="/app/deals/create" aria-label="Create deal">
    ...
    <aside class="drawer" :class="{ open: drawerOpen }" aria-label="Workspace tools">
      ...
      <nav class="drawer-nav">
        ...
        <RouterLink class="tool-link" to="/dashboard/bookings" @click="drawerOpen = false">Bookings ledger</RouterLink>
        <RouterLink class="tool-link" to="/dashboard/settings" @click="drawerOpen = false">Settings</RouterLink>
        <RouterLink class="tool-link" to="/admin/overview" @click="drawerOpen = false">Admin ops</RouterLink>
        <RouterLink class="tool-link" to="/dashboard/payouts" @click="drawerOpen = false">Payout controls</RouterLink>
      </nav>
    </aside>
  </div>
</template>
...
<style scoped>
.app-shell { min-height: 100dvh; background: radial-gradient(900px 420px at 15% -10%, rgba(38, 91, 169, 0.18), transparent 60%), linear-gradient(180deg, #081a32, #030b18); color: #e8eef8; padding-bottom: calc(106px + env(safe-area-inset-bottom, 0px)); }
...
.fab-create {
  ...
  place-items: center;
  color: #0c1728;
  border: 1px solid rgba(240,190,100,.45);
  background: linear-gradient(145deg, #f3d89f, #e9c57b);
  box-shadow: 0 14px 28px rgba(0,0,0,.42), 0 0 0 1px rgba(240,190,100,.18);
  text-decoration: none;
  transition: transform .12s ease, box-shadow .12s ease, filter .12s ease;
}
.fab-create .icon { width: 20px; height: 20px; display: grid; }
.fab-create .icon :deep(svg) { width: 20px; height: 20px; }
.fab-create:hover {
...
.fab-create:active {
  ...
  box-shadow: 0 8px 20px rgba(0,0,0,.36), 0 0 0 1px rgba(240,190,100,.14);
}
